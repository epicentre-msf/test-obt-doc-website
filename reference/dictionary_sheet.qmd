...
::: callout-note
To add more details, you can use the following fields: [Sub Label](#sec-sub-label) and
[Note](sec-note).
:::
...
-   **Format**: calculated columns created for applying [conditional
    formatting](#sec-conditional-formating). These columns are very
...
Feel free to use this column in a ways that helps you work, or to ignore
it!
...
This part of the table organises variables within **sheets** and
...
more sheets can be added if you choose to include [analyses](../reference/analyses_sheet.qmd].
:::
...
-   **vlist1D**: corresponds to a table built *Vertically*, so the variables labels are in the same column, and **only one entry is expected for each variable**. This structure of data is well adapted
    to collect metadata about the health structure, the project or the data collection itself.
...
Variables can be shown or hidden using the [*Show/Hide* button](../how_to/show_hide_variables.qmd) in the **linelist file**.
...
a behaviour is needed (see the [Exports sheet](../reference/exports_sheet.qmd)).
...
*displayed*, which we here call [variable format](#sec-var-format). For example: 
...
enter any format by hand, as long as it is written in **Excel syntax** (see [here](https://www.ablebits.com/office-addins-blog/excel-format-number-text/) and [here](https://www.ablebits.com/office-addins-blog/custom-excel-number-format/) for information).
...
These options broadly correspond to two sorts of variables to
be created: 
...
::: {.callout-note}
A choice_multiple column cannot be used in the [Analyses sheets](../reference/analyses_sheets.qmd) as the designer cannot predict the number of combinations the user will create, and thus cannot create tables with appropriate size.
:::
...
While by default the dropdown menu created is empty, it is possible to pre-fill it by filling the "Control Details" column with a list of choices, like for the other types of dropdowns menus (see
the [details on setting up dropdown menus](#sec-var-control-details section) below). The table in the *Custom dropdown* sheet of the linelist will still be created and future users will be able to modify it.
...
linelist, this results in the creation of **four columns**, one for each
...
These values are specified in the [Choices sheet](../reference/choices_sheet.qmd). Categories of a same variable are bundled into named *lists*. The name of the list is then entered in
...
to populate the dropdown menu. See [this How to](../how_to/link_tables.qmd#sec-ht-list_auto) for an example.


#### Formulas {#sec-formulas}

If you have selected "formula", "choice_formula" or "case_when" in the
[Control](#sec-var-control) column, you need to provide the formula to
...
The formula is written **in English, without an "=" sign**. During compilation, the
formulas will automatically be translated into the Excel language defined on
the computer used to generate the linelist.

::: {.callout-warning}
If your Excel is not configured to use English, pay special attention to the
following:
...
::: {.callout-tip}
In Excel, you can use `ALT+ENTRY` to insert a carriage return to go to
...
block the user if they attempt to enter invalid data.

For continuous variables, you can to specify the `min` and or `max` values
(see below). For categorical variables with dropdown menus, go directly to the [Alert](#sec-alert) column.
...
- `date_notification` to not allow the date of exit to be before the
...
❌ The "error" option blocks data entry: it is impossible to force the
...
expected values, but if they wish they can still force this data in. It is
