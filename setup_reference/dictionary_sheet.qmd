...
The metadata about linelist variable, which is commonly called the *variable dictionary* is stored in two of the **setup file** sheets, the *Dictionary* sheet and the [*Choices* sheet](../setup_reference/choices_sheet.qmd). Most of the variable attributes/metadata are stored in the *Dictionary* sheet, while only the categories of categorical variables are stored in the *Choices* sheet.
...
This is a free text field used to **uniquely identify the variables**. The name can be used to refer to the variable in formulas or in the [*Analyses* sheet](../setup_reference/analyses_sheet.qmd).
...
The created linelist will contain more sheets than the one you specify in this section, as some of the sheets are added automatically during compilation (an instruction sheet, a sheet for the geobase, etc.) and more sheets can be added if you choose to include [analyses](../setup_reference/analyses_sheet.qmd).
:::
...
Variables can be shown or hidden using the "Show/Hide" button of the [linelist OutbreakTools menu](../setup_reference/ribbons.qmd#sec-linelist-ribbon) in the **linelist file**. See the [how to](../how_to/show_hide_variables.qmd).
...
- it is used to remove variables automatically from the exports if such a behaviour is needed (see the [Exports sheet](../setup_reference/exports_sheet.qmd)).
...
A choice_multiple column cannot be used in the [Analyses sheets](../setup_reference/analyses_sheet.qmd) as the designer cannot predict the number of combinations the user will create, and thus cannot create tables with appropriate size.
:::
...
- the name of a *list* (defined in the [Choices](../setup_reference/choices_sheet.qmd) sheet), 
...
These values are specified in the [Choices sheet](../setup_reference/choices_sheet.qmd). Categories of a same variable are bundled into named *lists*. The name of the list is then entered in the "Control details" column of the *Dictionary* sheet.
...
To define a specific export, you need to fill both the "Export" section in the *Dictionary* sheet and the [Exports](../setup_reference/exports_sheet/qmd) sheet. The *Export* sheet contains metadata for each export, while the "Export" columns in the *Dictionary* sheet allows to decide individually for each variable whether to export it or not in a given export.

To create a new export, first fill the [Exports](../setup_reference/exports_sheet/qmd) sheet, which will add corresponding columns to the *Dictionary* sheet.
