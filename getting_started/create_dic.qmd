...
Aka fill the [Dictionary](../setup_reference/dictionary_sheet.qmd) and [Choices](../setup_reference/choices_sheet.qmd) sheets. The first sheet is a database about all the *variables* present in the linelist you want to create. The *Choices* sheet contains the categories for categorical variables.
...
Fill the [Exports](../setup_reference/exports_sheet.qmd) sheet to **define one or more personalised exports**: their names, pseudo-anonymisation, the label of their button, whether to export variable names or labels, how much metadata to export etc.

Fill the [Analyses](../setup_reference/analyses_sheet.qmd) sheet to **define basic analyses** to include in the linelist:
...
Fill the [Translations](../setup_reference/analyses_sheet.qmd) sheet to translate strings of text so that the **linelist can automatically be built in other languages** than the one in the setup.
...
To do that, go to the "Advanced" section of the [OutbreakTools ribbon](../setup_reference/ribbons.qmd#sec-linelist-ribbon) and click on the "Export for migration" button. This will create a data export, that can be later imported in a new empty linelist by clicking on the "Import data" button from the same ribbon.
:::
