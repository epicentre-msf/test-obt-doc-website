...
### Change style
You can select the other "Linelist design" using the dropdown menu (see the [ribbon](../reference/ribbons.qmd#sec-setup-styles) to import your own style)


### Import a geobase

If you are compiling a linelist for a given country, you could generate the linelist with the geobase already imported. Click on the "Import de geobase" button to import a geobase in excel format (learn [how to get](../how_to/import_geobase#sec-geobase-download) a geobase) before compiling.
...
There are a couple more customisations available through the [OutbreakTools option ribbon](../reference/ribbons.qmd#sec-designer-ribbon).

![OutbreakTools tab in the designer file](../images/img-003-ribbon-designer.png){fig-align="center"}

## Compilation advice

The linelist compilation relies on Excel VBA and can be a litle bit finicky. Here is some advice.

- [Run the checks](../reference/ribbons.qmd#sec-check-setup) on the setup before attempting compilation to verify that everything is good.

- Make sure there are no hidden columns or filters on the sheets of the setup.
...
- If it fails, start with a new designer file, in case the other got corrupted when compilation fails.
...
::: {.callout-tip}
One of the writer of this guide prepares the setup, the designer, and then copy both of them and use the copies to compile. If a compilation fails, delete both copies and start from fresh, just in case the failure to compile corrupted any of the file.
:::