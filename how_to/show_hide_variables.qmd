...
In OutbreakTools, there are several visibility status, which can be modified using a button in the [OutbreakTools Menu](../reference/ribbons.qmd#sec-linelist-ribbon):
...
These linelist status correspond to the **setup file** [status property](../reference/dictionary_sheet.qmd#sec-visibility-status), with the subtility that the setup user can decide for optional columns to show them by default (*optional, visible*) or hide them by default (*optional, hidden*), based on the expected frequency of use of the variable.
