...
Both levels of translation are selected during the [compilation](../getting_started/compilation.qmd) steps of a given linelist.


## Add a new language to a setup

Go to the [OutbreakTools tab](../reference/ribbons.qmd#sec-setup-ribbon) of the setup top ribbon. One section contains buttons for translation:
...
1. Click on the "Update translation table", which is going to scrap all text bits from the setup and add them to the translation table (even if there is only one language in the sheet). This step can be done at a later stage, and each time you modify a label in the setup.
