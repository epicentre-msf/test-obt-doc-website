...
- In the *Linelist patients* sheet, add one or two patients manually. In particular, use the geo-Helper to enter the patients' origin (see *Data table* section of the [OutbreakTools ribbon](setup_reference/ribbons.qmd#sec-linelist-ribbon)).

- Explore univariate, bivariate, temporal and spatial analyses. Use button "Refresh analyses" in the [OutbreakTools ribbon](setup_reference/ribbons.qmd#sec-linelist-ribbon) to update the analyses.
...
- Export the filtered data using the `anonymous export for MSF` (see *Export button* in the [OutbreakTools ribbon](setup_reference/ribbons.qmd#sec-linelist-ribbon)). Open and browse the exported file.

- Finally, print a register book for the `Linelist patients` (see the *Register book* section of the [OutbreakTools ribbon](setup_reference/ribbons.qmd#sec-linelist-ribbon)).
...
- Translate newly added labels into French (see  the [Setup Options ribbon](setup_reference/ribbons.qmd#sec-setup-ribbon) and the [Setup Translation sheet](setup_reference/translations_sheet.qmd)).

- Check the setup for potential errors (see  the [Setup Options ribbon](setup_reference/ribbons.qmd#sec-setup-ribbon)).
...
- You are now ready to migrate your data from the old to the new linelist. You need to proceed in 2 steps:
  - First, go back to the linelist `linelist_measles_en_ex1.xlsb` and remove all filters (see the [linelist options ribbon](setup_reference/ribbons.qmd#sec-linelist-ribbon)). Then, export the data (use the *Export data* button in the same ribbon) to a location of your choice. This migration file contains all your data.
  
  - Second, go back to the new linelist `linelist_measles_en_ex2.xlsb` and import the migration file (see the button "Import data" from the [linelist options ribbon](setup_reference/ribbons.qmd#sec-linelist-ribbon)). Look at the importation report and browse the data in the new linelist.
